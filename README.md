...
- **GitHub レビュー**: 自分へのレビュー依頼または指定したPRから、対象ディレクトリのデフォルトブランチとの差分を開いたAIセッションを開始
...
差分モードでは、セッションの作業ディレクトリをリポジトリのデフォルトブランチと比較します。
...
対象PRを紐づけて差分モードを最初から開きます（差分の比較元はPRではなくデフォルトブランチです）。
